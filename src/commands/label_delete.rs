...
#[derive(Debug, Parser)]
/// Delete a label from all repositories that match a regex
...
impl LabelDeleteArgs {
    ...
    fn run_for_owner(&self, owner: &str) -> Result<OrgResult> {
        ...
        let results: Vec<_> = filtered_repos
            .par_iter()
            .map(|repo| {
                let result = github::delete_label(repo, &self.name, &user_token);
                match result {
                    Ok(()) => {
                        println!("Deleted label '{}' from repo {}", self.name, repo.name);
                        true
                    }
                    Err(e) => {
                        println!(
                            "Failed to delete label from repo {} because {:?}",
                            repo.name, e
                        );
                        false
                    }
